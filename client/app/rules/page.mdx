...
3. Gen 2 timers are allowed in addition to the timers allowed for WCA
   competitions.
4. All scrambleable puzzles must be scrambled using approved software. Unless specified otherwise,
   [cubing.js](https://experiments.cubing.net/cubing.js/mark3/),
   [csTimer](https://cstimer.net/), or
   [TNoodle](https://www.worldcubeassociation.org/regulations/scrambles/) scrambles must be used, if available.
   If there is no scrambler available that is specific for the event, please contact the admin team before holding the event.
...
8. Contest names and IDs may not include the words "championship", "national",
   or "nationals".
9. Contests may hold no more than 30 events in total (no more than 15 for meetups).
